odoo.define('makemyday_makemyday.floors', function (require) {
"use strict";
...
models.load_models({
    ...
    domain: function(self){ return [['makemyday_config_id','=',self.config.id]]; },
...
var TableWidget = PosBaseWidget.extend({
    ...
    click_handler: function(){
        ...
        if (floorplan.editing) {
            ...
            floorplan.makemyday.set_table(this.table);
        }
    },
    ...
    save_changes: function(){
        var self   = this;
        var model  = new Model('makemyday.table');
        var fields = _.find(this.makemyday.models,function(model){ return model.model === 'makemyday.table'; }).fields;
    ...
    trash: function(){
        ...
        return model.call('create_from_ui',[{'active':false,'id':this.table.id}]).then(function(table_id){
            ...
            for (var i = 0; i < self.makemyday.floors.length; i++) {
                var floor = self.makemyday.floors[i];
    ...
    get_notifications: function(){  //FIXME : Make this faster
        var orders = this.makemyday.get_table_orders(this.table);
        var notifications = {};
    ...
    renderElement: function(){
        var self = this;
        this.order_count    = this.makemyday.get_table_orders(this.table).length;
        this.customer_count = this.makemyday.get_customer_count(this.table);
...
var FloorScreenWidget = screens.ScreenWidget.extend({
    ...
    init: function(parent, options) {
        this._super(parent, options);
        this.floor = this.makemyday.floors[0];
        this.table_widgets = [];
    ...
    click_floor_button: function(event,$el){
        var floor = this.makemyday.floors_by_id[$el.data('id')];
        if (floor !== this.floor) {
            if (this.editing) {
                this.toggle_editing();
            }
    ...
    background_image_url: function(floor) { 
        return '/web/image?model=makemyday.floor&id='+floor.id+'&field=background_image';
    },
...
gui.define_screen({
    ...
    'condition': function(){
        return this.makemyday.config.iface_floorplan;
    },
});
...
chrome.Chrome.include({
    build_widgets: function(){
        this._super();
        if (this.makemyday.config.iface_floorplan) {
...
models.Order = models.Order.extend({
    initialize: function() {
        ...
        if (!this.table) {
            this.table = this.makemyday.table;
        }
    ...
    init_from_JSON: function(json) {
        ...
        this.table = this.makemyday.tables_by_id[json.table_id];
        this.floor = this.table ? this.makemyday.floors_by_id[json.floor_id] : undefined;
    ...
    renderElement: function(){
        ...
        if (this.makemyday.config.iface_floorplan) {
            if (this.makemyday.get_order()) {
                if (this.makemyday.table && this.makemyday.table.floor) {
                    this.$('.orders').prepend(QWeb.render('BackToFloorButton',{table: this.makemyday.table, floor:this.makemyday.table.floor}));
...
var TableGuestsButton = screens.ActionButtonWidget.extend({
    ...
    guests: function() {
        if (this.makemyday.get_order()) {
            return this.makemyday.get_order().customer_count;
        } else {
            return 0;
        }
    },
    button_click: function() {
        ...
        this.gui.show_popup('number', {
            'title':  _t('Guests ?'),
            'cheap': true,
            'value':   this.makemyday.get_order().customer_count,
            'confirm': function(value) {
                value = Math.max(1,Number(value));
                self.makemyday.get_order().set_customer_count(value);
                self.renderElement();
            },
        });
    },
