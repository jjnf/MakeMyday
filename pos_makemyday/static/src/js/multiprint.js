odoo.define('makemyday_makemyday.multiprint', function (require) {
"use strict";
...
models.Orderline = models.Orderline.extend({
    initialize: function() {
        ...
        if (!this.makemyday.config.iface_printers) {
            return;
        }
    ...
    printable: function() {
        return this.makemyday.db.is_product_in_category(this.makemyday.printers_categories, this.get_product().id);
    },
    ...
    set_quantity: function(quantity) {
        if (this.makemyday.config.iface_printers && quantity !== this.quantity && this.printable()) {
            this.mp_dirty = true;
        }
...
screens.OrderWidget.include({
    ...
    click_line: function(line, event) {
        ...
        } else if (this.makemyday.get_order().selected_orderline !== line) {
...
models.Order = models.Order.extend({
    ...
    computeChanges: function(categories){
        ...
        for ( line_hash in current_res) {
            ...
            if (typeof old === 'undefined') {
                add.push({
                    'id':       curr.product_id,
                    'name':     this.makemyday.db.get_product_by_id(curr.product_id).display_name,
                    'note':     curr.note,
                    'qty':      curr.qty,
                });
        ...
        for (line_hash in old_res) {
            if (typeof current_res[line_hash] === 'undefined') {
                ...
                rem.push({
                    'id':       old.product_id,
                    'name':     this.makemyday.db.get_product_by_id(old.product_id).display_name,
                    'note':     old.note,
                    'qty':      old.qty, 
                });
            }
        ...
        if(categories && categories.length > 0){
            ...
            for(var i = 0; i < add.length; i++){
                if(self.makemyday.db.is_product_in_category(categories,add[i].id)){
                    _add.push(add[i]);
                }
            }
            add = _add;
            ...
            for(var i = 0; i < rem.length; i++){
                if(self.makemyday.db.is_product_in_category(categories,rem[i].id)){
                    _rem.push(rem[i]);
                }
            }
            rem = _rem;
    ...
    printChanges: function(){
        var printers = this.makemyday.printers;
        for(var i = 0; i < printers.length; i++){
...
screens.define_action_button({
    ...
    'condition': function() {
        return this.makemyday.printers.length;
    },
});
...
screens.OrderWidget.include({
    update_summary: function(){
        this._super();
        var changes = this.makemyday.get_order().hasChangesToPrint();
        var skipped = changes ? false : this.makemyday.get_order().hasSkippedChanges();
