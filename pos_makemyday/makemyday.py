...
class makemyday_floor(osv.osv):
    _name = 'makemyday.floor'
    _columns = {
        'name':             fields.char('Floor Name', required=True, help='An internal identification of the restaurant floor'),
        'makemyday_config_id':    fields.many2one('makemyday.config','Point of Sale'),
        ...
        'table_ids':        fields.one2many('makemyday.table','floor_id','Tables', help='The list of tables in this floor'),
...
class makemyday_table(osv.osv):
    _name = 'makemyday.table'
    _columns = {
        'name':         fields.char('Table Name', size=32, required=True, help='An internal identification of a table'),
        'floor_id':     fields.many2one('makemyday.floor','Floor'),
        ...
        'makemyday_order_ids':fields.one2many('makemyday.order','table_id','Pos Orders', help='The orders served at this table'),
    }
...
class makemyday_printer(osv.osv):
    _name = 'makemyday.printer'
    ...
    _columns = {
        ...
        'product_categories_ids': fields.many2many('makemyday.category','printer_category_rel', 'printer_id','category_id',string='Printed Product Categories'),
    }
...
class makemyday_config(osv.osv):
    ...
    _columns = {
        ...
        'floor_ids':       fields.one2many('makemyday.floor','makemyday_config_id','Restaurant Floors', help='The restaurant floors served by this point of sale'),
        'printer_ids':     fields.many2many('makemyday.printer','makemyday_config_printer_rel', 'config_id','printer_id',string='Order Printers'),
    }
    _defaults = {
...
class makemyday_order(osv.osv):
    _inherit = 'makemyday.order'
    _columns = {
        'table_id': fields.many2one('makemyday.table','Table', help='The table where this order was served'),
        'customer_count' : fields.integer('Guests', help='The amount of customers that have been served by this order.'),
    }

    def _order_fields(self, cr, uid, ui_order, context=None):
        fields = super(makemyday_order,self)._order_fields(cr,uid,ui_order,context)
