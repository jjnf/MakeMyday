...
class makemyday_config(osv.osv):
    ...
    def _get_currency(self, cr, uid, ids, fieldnames, args, context=None):
        ...
        for makemyday_config in self.browse(cr, uid, ids, context=context):
            if makemyday_config.journal_id:
                currency_id = makemyday_config.journal_id.currency_id.id or makemyday_config.journal_id.company_id.currency_id.id
            else:
                currency_id = self.pool['res.users'].browse(cr, uid, uid, context=context).company_id.currency_id.id
            result[makemyday_config.id] = currency_id
        return result
    ...
    def _get_last_session(self, cr, uid, ids, fieldnames, args, context=None):
        ...
        for record in self.browse(cr, uid, ids, context=context):
            session_ids = self.pool['makemyday.session'].search_read(
                cr, uid,
    ...
    _columns = {
        ...
        'journal_ids' : fields.many2many('account.journal', 'makemyday_config_journal_rel', 
             'makemyday_config_id', 'journal_id', 'Available Payment Methods',
        ...
        'iface_start_categ_id': fields.many2one('makemyday.category','Start Category', help='The point of sale will display this product category by default. If no category is specified, all available products will be shown'),
        ...
        'session_ids': fields.one2many('makemyday.session', 'config_id', 'Sessions'),
        'current_session_id': fields.function(_get_current_session, multi="session", type="many2one", relation="makemyday.session", string="Current Session"),
        ...
        'makemyday_session_username': fields.function(_get_current_session_user, type='char'),
    ...
    def _get_group_makemyday_manager(self, cr, uid, context=None):
        group = self.pool.get('ir.model.data').get_object_reference(cr,uid,'point_of_sale','group_makemyday_manager')
        if group:
            return group[1]
        else:
            return False

    def _get_group_makemyday_user(self, cr, uid, context=None):
        group = self.pool.get('ir.model.data').get_object_reference(cr,uid,'point_of_sale','group_makemyday_user')
        if group:
            return group[1]
        else:
            return False
    ...
    _defaults = {
        ...
        'group_makemyday_manager_id': _get_group_makemyday_manager,
        'group_makemyday_user_id': _get_group_makemyday_user,
    }
    ...
    def create(self, cr, uid, values, context=None):
        ...
        ir_sequence.create(cr, SUPERUSER_ID, {
            ...
            'padding': 4,
            'prefix': "%s/"  % values['name'],
            'code': "makemyday.order.line",
            'company_id': values.get('company_id', False),
        }, context=context)

        return super(makemyday_config, self).create(cr, uid, values, context=context)

    def unlink(self, cr, uid, ids, context=None):
        for obj in self.browse(cr, uid, ids, context=context):
            if obj.sequence_id:
                obj.sequence_id.unlink()
        return super(makemyday_config, self).unlink(cr, uid, ids, context=context)
    ...
    def open_session_cb(self, cr, uid, ids, context=None):
        ...
        proxy = self.pool.get('makemyday.session')
        record = self.browse(cr, uid, ids[0], context=context)
    ...
    def _open_session(self, session_id):
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form,tree',
            'res_model': 'makemyday.session',
            'res_id': session_id,
            'view_id': False,
            'type': 'ir.actions.act_window',
        }

class makemyday_session(osv.osv):
    _name = 'makemyday.session'
    _order = 'id desc'
    ...
    _columns = {
        'config_id' : fields.many2one('makemyday.config', 'Point of Sale',
        ...
        'order_ids' : fields.one2many('makemyday.order', 'session_id', 'Orders'),

        'statement_ids' : fields.one2many('account.bank.statement', 'makemyday_session_id', 'Bank Statement', readonly=True),
    }
    ...
    def _check_makemyday_config(self, cr, uid, ids, context=None):
        for session in self.browse(cr, uid, ids, context=None):
            domain = [
                ('state', '!=', 'closed'),
    ...
    _constraints = [
        ...
        (_check_makemyday_config, "You cannot create two active sessions related to the same point of sale!", ['config_id']),
    ]
    ...
    def create(self, cr, uid, values, context=None):
        ...
        # journal_id is not required on the makemyday_config because it does not
        # exists at the installation. If nothing is configured at the
        # installation we do the minimal configuration. Impossible to do in
        # the .xml files as the CoA is not yet installed.
        jobj = self.pool.get('makemyday.config')
        makemyday_config = jobj.browse(cr, uid, config_id, context=context)
        context.update({'company_id': makemyday_config.company_id.id})
        if not makemyday_config.journal_id:
            jid = jobj.default_get(cr, uid, ['journal_id'], context=context)['journal_id']
            if jid:
                jobj.write(cr, SUPERUSER_ID, [makemyday_config.id], {'journal_id': jid}, context=context)
            else:
                raise UserError(_("Unable to open the session. You have to assign a sale journal to your point of sale."))

        # define some cash journal if no payment method exists
        if not makemyday_config.journal_ids:
            ...
            jobj.write(cr, SUPERUSER_ID, [makemyday_config.id], {'journal_ids': [(6,0, cashids)]})
        ...
        statements = [(0, 0, {
            ...
            'user_id': uid,
            'company_id': makemyday_config.company_id.id
        }) for journal in makemyday_config.journal_ids]

        values.update({
            'name': self.pool['ir.sequence'].next_by_code(cr, uid, 'makemyday.session', context=context),
        ...
        if makemyday_config.cash_control:
            for journal in makemyday_config.journal_ids:
                if journal.type == 'cash':
                    context.update({'journal_id': journal.id})

        return super(makemyday_session, self).create(cr, uid, values, context=context)

    def unlink(self, cr, uid, ids, context=None):
        for obj in self.browse(cr, uid, ids, context=context):
            self.pool.get('account.bank.statement').unlink(cr, uid, obj.statement_ids.ids, context=context)
        return super(makemyday_session, self).unlink(cr, uid, ids, context=context)
    ...
    def _confirm_orders(self, cr, uid, ids, context=None):
        makemyday_order_obj = self.pool.get('makemyday.order')
        for session in self.browse(cr, uid, ids, context=context):
            company_id = session.config_id.journal_id.company_id.id
            local_context = dict(context or {}, force_company=company_id)
            order_ids = [order.id for order in session.order_ids if order.state == 'paid']

            move_id = makemyday_order_obj._create_account_move(cr, uid, session.start_at, session.name, session.config_id.journal_id.id, company_id, context=context)

            makemyday_order_obj._create_account_move_line(cr, uid, order_ids, session, move_id, context=local_context)
            ...
            for order in session.order_ids:
                ...
                if order.state not in ('paid', 'invoiced'):
                    ...
                    makemyday_order_obj.signal_workflow(cr, uid, [order.id], 'done')
...
class makemyday_order(osv.osv):
    ...
    def _order_fields(self, cr, uid, ui_order, context=None):
        process_line = partial(self.pool['makemyday.order.line']._order_line_fields, cr, uid, context=context)
        return {
            'name':         ui_order['name'],
            'user_id':      ui_order['user_id'] or False,
            'session_id':   ui_order['makemyday_session_id'],
            'lines':        [process_line(l) for l in ui_order['lines']] if ui_order['lines'] else False,
            'makemyday_reference':ui_order['name'],
    ...
    def _get_valid_session(self, cr, uid, order, context=None):
        session = self.pool.get('makemyday.session')
        closed_session = session.browse(cr, uid, order['makemyday_session_id'], context=context)
    ...
    def _process_order(self, cr, uid, order, context=None):
        session = self.pool.get('makemyday.session').browse(cr, uid, order['makemyday_session_id'], context=context)

        if session.state == 'closing_control' or session.state == 'closed':
            session_id = self._get_valid_session(cr, uid, order, context=context)
            session = self.pool.get('makemyday.session').browse(cr, uid, session_id, context=context)
            order['makemyday_session_id'] = session_id
    ...
    def create_from_ui(self, cr, uid, orders, context=None):
        ...
        existing_order_ids = self.search(cr, uid, [('makemyday_reference', 'in', submitted_references)], context=context)
        existing_orders = self.read(cr, uid, existing_order_ids, ['makemyday_reference'], context=context)
        existing_references = set([o['makemyday_reference'] for o in existing_orders])
    ...
    def write(self, cr, uid, ids, vals, context=None):
        res = super(makemyday_order, self).write(cr, uid, ids, vals, context=context)
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        return super(makemyday_order, self).unlink(cr, uid, ids, context=context)
    ...
    _columns = {
        ...
        'lines': fields.one2many('makemyday.order.line', 'order_id', 'Order Lines', states={'draft': [('readonly', False)]}, readonly=True, copy=True),
        'statement_ids': fields.one2many('account.bank.statement.line', 'makemyday_statement_id', 'Payments', states={'draft': [('readonly', False)]}, readonly=True),
        ...
        'session_id' : fields.many2one('makemyday.session', 'Session', 
                                        required=True,
                                        select=1,
                                        domain="[('state', '=', 'opened')]",
                                        states={'draft' : [('readonly', False)]},
                                        readonly=True),
        'config_id': fields.related('session_id', 'config_id', string="Point of Sale", type='many2one'  , relation='makemyday.config'),
        ...
        'makemyday_reference': fields.char('Receipt Ref', readonly=True, copy=False),
    ...
    def _default_session(self, cr, uid, context=None):
        so = self.pool.get('makemyday.session')
    ...
    def _default_pricelist(self, cr, uid, context=None):
        ...
        if session_ids:
            session_record = self.pool.get('makemyday.session').browse(cr, uid, session_ids, context=context)
    ...
    def create(self, cr, uid, values, context=None):
        if values.get('session_id'):
            ...
            session = self.pool['makemyday.session'].browse(cr, uid, values['session_id'], context=context)
            values['name'] = session.config_id.sequence_id._next()
            values.setdefault('session_id', session.config_id.pricelist_id.id)
        else:
            # fallback on any makemyday.order sequence
            values['name'] = self.pool.get('ir.sequence').next_by_code(cr, uid, 'makemyday.order', context=context)
        return super(makemyday_order, self).create(cr, uid, values, context=context)
    ...
    def create_picking(self, cr, uid, ids, context=None):
        ...
        for order in self.browse(cr, uid, ids, context=context):
            ...
            if picking_type:
                makemyday_qty = all([x.qty >= 0 for x in order.lines])
                ...
                picking_id = picking_obj.create(cr, uid, {
                    ...
                    'move_type': 'direct',
                    'note': order.note or "",
                    'location_id': location_id if makemyday_qty else destination_id,
                    'location_dest_id': destination_id if makemyday_qty else location_id,
                }, context=context)
    ...
    def add_payment(self, cr, uid, order_id, data, context=None):
        ...
        context.pop('makemyday_session_id', False)
        ...
        args.update({
            ...
            'makemyday_statement_id': order_id,
    ...
    def refund(self, cr, uid, ids, context=None):
        ...
        clone_list = []
        line_obj = self.pool.get('makemyday.order.line')
        
        for order in self.browse(cr, uid, ids, context=context):
            current_session_ids = self.pool.get('makemyday.session').search(cr, uid, [
                ('state', '!=', 'closed'),
    ...
    def action_invoice(self, cr, uid, ids, context=None):
        ...
        for order in self.pool.get('makemyday.order').browse(cr, uid, ids, context=context):
...
class account_bank_statement_line(osv.osv):
    ...
    _columns= {
        'makemyday_statement_id': fields.many2one('makemyday.order', string="POS statement", ondelete='cascade'),
    }

class makemyday_order_line(osv.osv):
    _name = "makemyday.order.line"
    ...
    _columns = {
        ...
        'price_subtotal': fields.function(_amount_line_all, multi='makemyday_order_line_amount', digits=0, string='Subtotal w/o Tax'),
        'price_subtotal_incl': fields.function(_amount_line_all, multi='makemyday_order_line_amount', digits=0, string='Subtotal'),
        'discount': fields.float('Discount (%)', digits=0),
        'order_id': fields.many2one('makemyday.order', 'Order Ref', ondelete='cascade'),
    ...
    _defaults = {
        'name': lambda obj, cr, uid, context: obj.pool.get('ir.sequence').next_by_code(cr, uid, 'makemyday.order.line', context=context),
        'qty': lambda *a: 1,
        'discount': lambda *a: 0.0,
        'company_id': lambda self,cr,uid,c: self.pool.get('res.users').browse(cr, uid, uid, c).company_id.id,
    }

class makemyday_category(osv.osv):
    _name = "makemyday.category"
    ...
    _columns = {
        ...
        'parent_id': fields.many2one('makemyday.category','Parent Category', select=True),
        'child_id': fields.one2many('makemyday.category', 'parent_id', string='Children Categories'),
