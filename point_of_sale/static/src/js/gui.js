...
var Gui = core.Class.extend({
    ...
    init: function(options){
        ...
        this.chrome.ready.then(function(){
            ...
            self.makemyday.bind('change:selectedOrder', function(){
                self.show_saved_screen(self.makemyday.get_order());
            });
        });
    },
    ...
    get_current_screen: function() {
        return this.makemyday.get_order() ? ( this.makemyday.get_order().get_screen_data('screen') || this.default_screen ) : this.startup_screen;
    },

    // goes to the previous screen (as specified in the order). The history only
    // goes 1 deep ...
    back: function() {
        var previous = this.makemyday.get_order().get_screen_data('previous-screen');
        if (previous) {
    ...
    get_current_screen_param: function(param) {
        if (this.makemyday.get_order()) {
            var params = this.makemyday.get_order().get_screen_data('params');
    ...
    close_other_tabs: function() {
        ...
        localStorage['message'] = JSON.stringify({
            'message':'close_tabs',
            'session': this.makemyday.makemyday_session.id,
        });
        ...
        window.addEventListener("storage", function(event) {
            ...
            if ( event.key === 'message' && event.newValue) {
                ...
                if ( msg.message  === 'close_tabs' &&
                     msg.session  ==  self.makemyday.makemyday_session.id ) {
    ...
    select_user: function(options){
        ...
        var list = [];
        for (var i = 0; i < this.makemyday.users.length; i++) {
            var user = this.makemyday.users[i];
        ...
        return def.then(function(user){
            if (options.security && user !== options.current_user && user.makemyday_security_pin) {
                return self.ask_password(user.makemyday_security_pin).then(function(){
                    return user;
                });
            } else {
                return user;
    ...
    sudo: function(user){
        user = user || this.makemyday.get_cashier();
    ...
    close: function() {
        var self = this;
        var pending = this.makemyday.db.get_orders().length;

        if (!pending) {
            this._close();
        } else {
            this.makemyday.push_order().always(function() {
                var pending = self.makemyday.db.get_orders().length;
                if (!pending) {
                    self._close();
                } else {
                    var reason = self.makemyday.get('failed') ? 
    ...
    _close: function() {
        ...
        this.makemyday.push_order().then(function(){
            return new Model("ir.model.data").get_func("search_read")([['name', '=', 'action_client_makemyday_menu']], ['res_id'])
            .pipe(function(res) {
