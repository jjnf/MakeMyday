...
var ScreenWidget = PosBaseWidget.extend({
    ...
    barcode_product_action: function(code){
        var self = this;
        if (self.makemyday.scan_product(code)) {
    ...
    barcode_cashier_action: function(code){
        var users = this.makemyday.users;
        for(var i = 0, len = users.length; i < len; i++){
            if(users[i].barcode === code.code){
                this.makemyday.set_cashier(users[i]);
    ...
    barcode_client_action: function(code){
        var partner = this.makemyday.db.get_partner_by_barcode(code.code);
        if(partner){
            this.makemyday.get_order().set_client(partner);
            return true;
        }
        this.barcode_error_action(code);
        return false;
    ...
    barcode_discount_action: function(code){
        var last_orderline = this.makemyday.get_order().get_last_orderline();
        if(last_orderline){
    ...
    show: function(){
        ...
        this.makemyday.barcode_reader.set_action_callback({
    ...
    close: function(){
        if(this.makemyday.barcode_reader){
            this.makemyday.barcode_reader.reset_action_callbacks();
        }
    },
...
var ScaleScreenWidget = ScreenWidget.extend({
    ...
    show: function(){
        this._super();
        var self = this;
        var queue = this.makemyday.proxy_queue;
        ...
        queue.schedule(function(){
            return self.makemyday.proxy.scale_read().then(function(weight){
    ...
    order_product: function(){
        this.makemyday.get_order().add_product(this.get_product(),{ quantity: this.weight });
    },
    ...
    get_product_weight_string: function(){
        ...
        var unit = this.makemyday.units_by_id[unit_id[0]];
    ...
    close: function(){
        ...
        this.makemyday.proxy_queue.clear();
    },
});
...
var ActionpadWidget = PosBaseWidget.extend({
    ...
    init: function(parent, options) {
        ...
        this.makemyday.bind('change:selectedClient', function() {
            self.renderElement();
        });
    },
    renderElement: function() {
...
var OrderWidget = PosBaseWidget.extend({
    ...
    init: function(parent, options) {
        ...
        this.makemyday.bind('change:selectedOrder', this.change_selected_order, this);

        this.line_click_handler = function(event){
            self.click_line(this.orderline, event);
        };

        if (this.makemyday.get_order()) {
            this.bind_order_events();
        }

    },
    click_line: function(orderline, event) {
        this.makemyday.get_order().select_orderline(orderline);
        this.numpad_state.reset();
    },
    ...
    remove_orderline: function(order_line){
        if(this.makemyday.get_order().get_orderlines().length === 0){
            this.renderElement();
        }else{
...
var ProductCategoriesWidget = PosBaseWidget.extend({
    ...
    init: function(parent, options){
        ...
        this.category = this.makemyday.root_category;
        this.breadcrumb = [];
        this.subcategories = [];
        this.product_list_widget = options.product_list_widget || null;
        this.category_cache = new DomCache();
        this.start_categ_id = this.makemyday.config.iface_start_categ_id ? this.makemyday.config.iface_start_categ_id[0] : 0;
        this.set_category(this.makemyday.db.get_category_by_id(this.start_categ_id));
        
        this.switch_category_handler = function(event){
            self.set_category(self.makemyday.db.get_category_by_id(Number(this.dataset.categoryId)));
            self.renderElement();
        };
    ...
    get_image_url: function(category){
        return window.location.origin + '/web/image?model=makemyday.category&field=image_medium&id='+category.id;
    },
    ...
    renderElement: function(){
        ...
        var withpics = this.makemyday.config.iface_display_categ_images;
        ...
        var products = this.makemyday.db.get_product_by_category(this.category.id); 
        ...
        if(this.makemyday.config.iface_vkeyboard && this.chrome.widget.keyboard){
    ...
    perform_search: function(category, query, buy_result){
        var products;
        if(query){
            products = this.makemyday.db.search_product_in_category(category.id,query);
            if(buy_result && products.length === 1){
                    this.makemyday.get_order().add_product(products[0]);
                    this.clear_search();
            }else{
                this.product_list_widget.set_product_list(products);
            }
        }else{
            products = this.makemyday.db.get_product_by_category(this.category.id);
            this.product_list_widget.set_product_list(products);
        }
    },
...
var ProductListWidget = PosBaseWidget.extend({
    ...
    init: function(parent, options) {
        ...
        this.click_product_handler = function(){
            var product = self.makemyday.db.get_product_by_id(this.dataset.productId);
...
var ProductScreenWidget = ScreenWidget.extend({
    ...
    start: function(){ 
        ...
        this.product_list_widget = new ProductListWidget(this,{
            ...
            product_list: this.makemyday.db.get_product_by_category(0)
        });
    ...
    click_product: function(product) {
       if(product.to_weight && this.makemyday.config.iface_electronic_scale){
           this.gui.show_screen('scale',{product: product});
       }else{
           this.makemyday.get_order().add_product(product);
       }
    },
...
var ClientListScreenWidget = ScreenWidget.extend({
    ...
    show: function(){
        ...
        this.old_client = this.makemyday.get_order().get_client();
        ...
        this.$('.new-customer').click(function(){
            self.display_client_details('edit',{
                'country_id': self.makemyday.company.country_id,
            });
        });

        var partners = this.makemyday.db.get_partners_sorted(1000);
    ...
    barcode_client_action: function(code){
        ...
        } else if (this.makemyday.db.get_partner_by_barcode(code.code)) {
            this.display_client_details('show',this.makemyday.db.get_partner_by_barcode(code.code));
        }
    },
    perform_search: function(query, associate_result){
        var customers;
        if(query){
            customers = this.makemyday.db.search_partner(query);
            ...
            customers = this.makemyday.db.get_partners_sorted();
            this.render_list(customers);
        }
    },
    clear_search: function(){
        var customers = this.makemyday.db.get_partners_sorted(1000);
        this.render_list(customers);
    ...
    save_changes: function(){
        if( this.has_client_changed() ){
            this.makemyday.get_order().set_client(this.new_client);
        }
    },
    ...
    line_select: function(event,$line,id){
        var partner = this.makemyday.db.get_partner_by_id(id);
    ...
    saved_client_details: function(partner_id){
        ...
        this.reload_partners().then(function(){
            var partner = self.makemyday.db.get_partner_by_id(partner_id);
            if (partner) {
    ...
    reload_partners: function(){
        var self = this;
        return this.makemyday.load_new_partners().then(function(){
            self.render_list(self.makemyday.db.get_partners_sorted(1000));
            
            // update the currently assigned client if it has been changed in db.
            var curr_client = self.makemyday.get_order().get_client();
            if (curr_client) {
                self.makemyday.get_order().set_client(self.makemyday.db.get_partner_by_id(curr_client.id));
            }
        });
    },
...
var ReceiptScreenWidget = ScreenWidget.extend({
    ...
    should_auto_print: function() {
        return this.makemyday.config.iface_print_auto && !this.makemyday.get_order()._printed;
    },
    should_close_immediately: function() {
        return this.makemyday.config.iface_print_via_proxy && this.makemyday.config.iface_print_skip_screen;
    },
    ...
    print_web: function() {
        window.print();
        this.makemyday.get_order()._printed = true;
    },
    print_xml: function() {
        var env = {
            widget:  this,
        ...
        this.makemyday.proxy.print_receipt(receipt);
        this.makemyday.get_order()._printed = true;
    },
    print: function() {
        var self = this;

        if (!this.makemyday.config.iface_print_via_proxy) { // browser (html) printing
    ...
    click_next: function() {
        this.makemyday.get_order().finalize();
    },
    click_back: function() {
    ...
    render_change: function() {
        this.$('.change-value').html(this.format_currency(this.makemyday.get_order().get_change()));
    },
...
var PaymentScreenWidget = ScreenWidget.extend({
    ...
    init: function(parent, options) {
        ...
        this.makemyday.bind('change:selectedOrder',function(){
                this.renderElement();
    ...
    reset_input: function(){
        var line = this.makemyday.get_order().selected_paymentline;
        this.firstinput  = true;
        if (line) {
	...
	var paymentlines = this.makemyday.get_order().get_paymentlines();
	...
	if (! open_paymentline) {
            this.makemyday.get_order().add_paymentline( this.makemyday.cashregisters[0]);
            this.render_paymentlines();
        }
    ...
    click_delete_paymentline: function(cid){
        var lines = this.makemyday.get_order().get_paymentlines();
        for ( var i = 0; i < lines.length; i++ ) {
            if (lines[i].cid === cid) {
                this.makemyday.get_order().remove_paymentline(lines[i]);
                this.reset_input();
                this.render_paymentlines();
                return;
            }
    ...
    click_paymentline: function(cid){
        ...
        for ( var i = 0; i < lines.length; i++ ) {
            if (lines[i].cid === cid) {
                this.makemyday.get_order().select_paymentline(lines[i]);
                this.reset_input();
                this.render_paymentlines();
                return;
            }
    ...
    click_paymentmethods: function(id) {
        ...
        for ( var i = 0; i < this.makemyday.cashregisters.length; i++ ) {
            if ( this.makemyday.cashregisters[i].journal_id[0] === id ){
                cashregister = this.makemyday.cashregisters[i];
                break;
            }
        }
        this.makemyday.get_order().add_paymentline( cashregister );
        this.reset_input();
        this.render_paymentlines();
    },
    ...
    customer_changed: function() {
        var client = this.makemyday.get_client();
    ...
    renderElement: function() {
        ...
        this.$('.js_cashdrawer').click(function(){
            self.makemyday.proxy.open_cashbox();
        });

    },
    show: function(){
        this.makemyday.get_order().clean_empty_paymentlines();
        this.reset_input();
        this.render_paymentlines();
        this.order_changes();
    ...
    validate_order: function(force_validation) {
        ...
        for (var i = 0; i < plines.length; i++) {
            if (plines[i].get_type() === 'bank' && plines[i].get_amount() < 0) {
                this.makemyday_widget.screen_selector.show_popup('error',{
        ...
        if (Math.abs(order.get_total_with_tax() - order.get_total_paid()) > 0.00001) {
            var cash = false;
            for (var i = 0; i < this.makemyday.cashregisters.length; i++) {
                cash = cash || (this.makemyday.cashregisters[i].journal.type === 'cash');
            }
            if (!cash) {
                this.gui.show_popup('error',{
        ...
        if (order.is_paid_with_cash() && this.makemyday.config.iface_cashdrawer) { 

                this.makemyday.proxy.open_cashbox();
        }

        if (order.is_to_invoice()) {
            var invoiced = this.makemyday.push_and_invoice_order(order);
            this.invoicing = true;
            ...
            this.makemyday.push_order(order);
...
var set_fiscal_position_button = ActionButtonWidget.extend({
    ...
    button_click: function () {
        var self = this;
        var selection_list = _.map(self.makemyday.fiscal_positions, function (fiscal_position) {
            return {
        ...
        self.gui.show_popup('selection',{
            ...
            confirm: function (fiscal_position) {
                var order = self.makemyday.get_order();
...
define_action_button({
    ...
    'condition': function(){
        return this.makemyday.fiscal_positions.length > 0;
    },
});
