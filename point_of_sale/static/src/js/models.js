...
exports.PosModel = Backbone.Model.extend({
    ...
    models: [
    {
        ...
        model:  'makemyday.session',
        fields: ['id', 'journal_ids','name','user_id','config_id','start_at','stop_at','sequence_number','login_number'],
        domain: function(self){ return [['state','=','opened'],['user_id','=',session.uid]]; },
        loaded: function(self,makemyday_sessions){
            self.makemyday_session = makemyday_sessions[0]; 
        },
    },{
        model: 'makemyday.config',
        fields: [],
        domain: function(self){ return [['id','=', self.makemyday_session.config_id[0]]]; },
        loaded: function(self,configs){
            ...
            for (var i = 0; i < orders.length; i++) {
                self.makemyday_session.sequence_number = Math.max(self.makemyday_session.sequence_number, orders[i].data.sequence_number+1);
            }
       },
    },{
        model:  'res.users',
        fields: ['name','makemyday_security_pin','groups_id','barcode'],
        domain: function(self){ return [['company_id','=',self.user.company_id[0]],'|', ['groups_id','=', self.config.group_makemyday_manager_id[0]],['groups_id','=', self.config.group_makemyday_user_id[0]]]; },
        loaded: function(self,users){ 
            ...
            for (var i = 0; i < users.length; i++) {
                ...
                for (var j = 0; j < user.groups_id.length; j++) {
                    ...
                    if (group_id === self.config.group_makemyday_manager_id[0]) {
                        user.role = 'manager';
                        break;
                    } else if (group_id === self.config.group_makemyday_user_id[0]) {
                        user.role = 'cashier';
                    }
                }
                if (user.role) {
                    makemyday_users.push(user);
                }
        ...
        fields: ['display_name', 'list_price','price','makemyday_categ_id', 'taxes_id', 'barcode', 'default_code', 
        ...
        fields: ['account_id','currency_id','journal_id','state','name','user_id','makemyday_session_id'],
        domain: function(self){ return [['state', '=', 'open'],['makemyday_session_id', '=', self.makemyday_session.id]]; },
    ...
    push_and_invoice_order: function(order){
        ...
        this.flush_mutex.exec(function(){
            ...
            transfer.pipe(function(order_server_id){    
                ...
                self.chrome.do_action('point_of_sale.makemyday_invoice_report',{additional_context:{ 
    ...
    export_paid_orders: function() {
        return JSON.stringify({
            ...
            'session':      this.makemyday_session.name,
            'session_id':    this.makemyday_session.id,
    ...
    import_orders: function(str) {
        ...
        if (json.unpaid_orders) {
            ...
            for (var i = 0; i < json.unpaid_orders.length; i++) {
                ...
                if (order.makemyday_session_id !== this.makemyday_session.id) {
                    report.unpaid_skipped_session += 1;
                    skipped_sessions[order.makemyday_session_id] = true;
...
exports.Orderline = Backbone.Model.extend({
    ...
    init_from_JSON: function(json) {
        this.product = this.makemyday.db.get_product_by_id(json.product_id);
        if (!this.product) {
        ...
        if(quantity === 'remove'){
            ...
            if(unit){
                if (unit.rounding) {
                    ...
                    var decimals = this.makemyday.dp['Product Unit of Measure'];
    ...
    set_unit_price: function(price){
        this.order.assert_editable();
        this.price = round_di(parseFloat(price) || 0, this.makemyday.dp['Product Price']);
        this.trigger('change',this);
    },
    ...
    get_unit_display_price: function(){
        if (this.makemyday.config.iface_tax_included) {
    ...
    get_base_price:    function(){
        var rounding = this.makemyday.currency.rounding;
    ...
    get_applicable_taxes: function(){
        ...
        var taxes = [];
        for (i = 0; i < this.makemyday.taxes.length; i++) {
            if (ptaxes_set[this.makemyday.taxes[i].id]) {
                taxes.push(this.makemyday.taxes[i]);
            }
        }
        return taxes;
    },
    ...
    get_taxes: function(){
        ...
        for (var i = 0; i < taxes_ids.length; i++) {
            taxes.push(this.makemyday.taxes_by_id[taxes_ids[i]]);
        }
        return taxes;
    },
    _map_tax_fiscal_position: function(tax) {
        var current_order = this.makemyday.get_order();
        ...
        if (order_fiscal_position) {
            ...
            if (mapped_tax) {
                tax = this.makemyday.taxes_by_id[mapped_tax.tax_dest_id[0]];
            }
        }
    ...
    compute_all: function(taxes, price_unit, quantity, currency_rounding) {
        ...
        var list_taxes = [];
        if (this.makemyday.company.tax_calculation_rounding_method == "round_globally"){
    ...
    get_all_prices: function(){
        ...
        var taxes =  this.makemyday.taxes;
        var taxdetail = {};
        var product_taxes = [];
        ...
        var all_taxes = this.compute_all(product_taxes, price_unit, this.get_quantity(), this.makemyday.currency.rounding);
...
exports.Paymentline = Backbone.Model.extend({
    ...
    init_from_JSON: function(json){
        ...
        this.cashregister = this.makemyday.cashregisters_by_id[json.statement_id];
        this.name = this.cashregister.journal_id[1];
    },
    //sets the amount of money on this payment line
    set_amount: function(value){
        this.order.assert_editable();
        this.amount = round_di(parseFloat(value) || 0, this.makemyday.currency.decimals);
        this.trigger('change',this);
    },
    ...
    get_amount_str: function(){
        return this.amount.toFixed(this.makemyday.currency.decimals);
    },
    set_selected: function(selected){
...
exports.Order = Backbone.Model.extend({
    initialize: function(attributes,options){
        ...
        this.makemyday_session_id = this.makemyday.makemyday_session.id;
        ...
        if (options.json) {
            ...
            this.sequence_number = this.makemyday.makemyday_session.sequence_number++;
    ...
    save_to_db: function(){
        if (!this.init_locked) {
            this.makemyday.db.save_unpaid_order(this);
        } 
    },
    init_from_JSON: function(json) {
        var client;
        this.sequence_number = json.sequence_number;
        this.makemyday.makemyday_session.sequence_number = Math.max(this.sequence_number+1,this.makemyday.makemyday_session.sequence_number);
        this.session_id    = json.makemyday_session_id;
        this.uid = json.uid;
        this.name = _t("Order ") + this.uid;
        if (json.partner_id) {
    ...
    export_as_JSON: function() {
        ...
        return {
            ...
            makemyday_session_id: this.makemyday_session_id,
            partner_id: this.get_client() ? this.get_client().id : false,
            user_id: this.makemyday.cashier ? this.makemyday.cashier.id : this.makemyday.user.id,
            uid: this.uid,
    ...
    export_for_printing: function(){
        ...
        var cashier = this.makemyday.cashier || this.makemyday.user;
        var company = this.makemyday.company;
        var shop    = this.makemyday.shop;
        var date    = new Date();
        ...
        var receipt = {
            ...
            header: this.makemyday.config.receipt_header || '',
            footer: this.makemyday.config.receipt_footer || '',
            precision: {
                price: 2,
                money: 2,
                quantity: 3,
            ...
            company:{
                ...
                name: company.name,
                phone: company.phone,
                logo:  this.makemyday.company_logo_base64,
            },
            shop:{
                name: shop.name,
            },
            currency: this.makemyday.currency,
        };
        
        if (is_xml(this.makemyday.config.receipt_header)){
            receipt.header_xml = render_xml(this.makemyday.config.receipt_header);
        }

        if (is_xml(this.makemyday.config.receipt_footer)){
            receipt.footer_xml = render_xml(this.makemyday.config.receipt_footer);
        }
    ...
    generate_unique_id: function() {
        ...
        return zero_pad(this.makemyday.makemyday_session.id,5) +'-'+
               zero_pad(this.makemyday.makemyday_session.login_number,3) +'-'+
    ...
    get_tip: function() {
        var tip_product = this.makemyday.db.get_product_by_id(this.makemyday.config.tip_product_id[0]);
    ...
    add_product: function(product, options){
        if(this._printed){
            this.destroy();
            return this.makemyday.get_order().add_product(product, options);
        }
        this.assert_editable();
        options = options || {};
    ...
    get_subtotal : function(){
        ...
        }), 0), this.makemyday.currency.rounding);
    },
    ...
    get_tax_details: function(){
        ...
        for(var id in details){
            if(details.hasOwnProperty(id)){
                fulldetails.push({amount: details[id], tax: this.makemyday.taxes_by_id[id], name: this.makemyday.taxes_by_id[id].name});
            }
        }
    ...
    get_total_for_category_with_tax: function(categ_id){
        ...
        this.orderlines.each(function(line){
            if ( self.makemyday.db.category_contains(categ_id,line.product.id) ) {
    ...
    get_change: function(paymentline) {
        ...
        return round_pr(Math.max(0,change), this.makemyday.currency.rounding);
    },
    get_due: function(paymentline) {
        ...
        return round_pr(Math.max(0,due), this.makemyday.currency.rounding);
    },
    is_paid: function(){
    ...
    destroy: function(){
        ...
        this.makemyday.db.remove_unpaid_order(this);
    },
    /* ---- Invoice --- */
