...
class makemyday_details(osv.osv_memory):
    _name = 'makemyday.details'
    _description = 'Sales Details'

    _columns = {
        'date_start': fields.date('Date Start', required=True),
        'date_end': fields.date('Date End', required=True),
        'user_ids': fields.many2many('res.users', 'makemyday_details_report_user_rel', 'user_id', 'wizard_id', 'Salespeople'),
    }
    _defaults = {
