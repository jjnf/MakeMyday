...
class makemyday_discount(osv.osv_memory):
    _name = 'makemyday.discount'
    ...
    def apply_discount(self, cr, uid, ids, context=None):
        """
         ...
         @return : nothing
        """
        order_ref = self.pool.get('makemyday.order')
        order_line_ref = self.pool.get('makemyday.order.line')
        if context is None:
            context = {}
