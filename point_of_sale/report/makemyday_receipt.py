...
class order(report_sxw.rml_parse):
    ...
    def netamount(self, order_line_id):
        sql = 'select (qty*price_unit) as net_price from makemyday_order_line where id = %s'
        self.cr.execute(sql, (order_line_id,))
        res = self.cr.fetchone()
        return res[0]

    def discount(self, order_id):
        sql = 'select discount, price_unit, qty from makemyday_order_line where order_id = %s '
    ...
    def _get_journal_amt(self, order_id):
        ...
        sql = """ select aj.name,absl.amount as amt from account_bank_statement as abs
                        ...
                        WHERE absl.makemyday_statement_id =%d"""%(order_id)
        self.cr.execute(sql)
