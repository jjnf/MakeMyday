...
class makemyday_details(report_sxw.rml_parse):
    ...
    def _makemyday_sales_details(self, form):
        makemyday_obj = self.pool.get('makemyday.order')
        user_obj = self.pool.get('res.users')
        data = []
        result = {}
        ...
        for pos in makemyday_obj.browse(self.cr, self.uid, makemyday_ids, context=self.localcontext):
            for pol in makemyday.lines:
                result = {
                    ...
                    'invoice_id': makemyday.invoice_id.id, 
                    'price_unit': pol.price_unit, 
                    'qty': pol.qty, 
                    'discount': pol.discount, 
                    'total': (pol.price_unit * pol.qty * (1 - (pol.discount) / 100.0)), 
                    'date_order': makemyday.date_order, 
                    'makemyday_name': makemyday.name, 
    ...
    def _get_payments(self, form):
        ...
        makemyday_ids = makemyday_order_obj.search(self.cr, self.uid, [('date_order','>=',form['date_start'] + ' 00:00:00'),('date_order','<=',form['date_end'] + ' 23:59:59'),('state','in',['paid','invoiced','done']),('user_id','in',user_ids), ('company_id', '=', company_id)])
        data={}
        if makemyday_ids:
            st_line_ids = statement_line_obj.search(self.cr, self.uid, [('makemyday_statement_id', 'in', makemyday_ids)])
            if st_line_ids:
    ...
    def _get_tax_amount(self, form):
        ...
        company_id = self.pool['res.users'].browse(self.cr, self.uid, self.uid).company_id.id
        makemyday_ids = makemyday_order_obj.search(self.cr, self.uid, [('date_order','>=',form['date_start'] + ' 00:00:00'),('date_order','<=',form['date_end'] + ' 23:59:59'),('state','in',['paid','invoiced','done']),('user_id','in',user_ids), ('company_id', '=', company_id)])
        for order in makemyday_order_obj.browse(self.cr, self.uid, makemyday_ids):
    ...
    def __init__(self, cr, uid, name, context):
        super(makemyday_details, self).__init__(cr, uid, name, context=context)
        self.total = 0.0
        self.qty = 0.0
        ...
        self.localcontext.update({
            ...
            'makemyday_sales_details':self._makemyday_sales_details,
