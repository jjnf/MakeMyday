...
class makemyday_user_product(report_sxw.rml_parse):

    def __init__(self, cr, uid, name, context):
        super(makemyday_user_product, self).__init__(cr, uid, name, context)
    ...
    def _get_data(self, o):
        self.total = 0.0
        data={}
        sql1=""" SELECT distinct(o.id) from account_bank_statement s, account_bank_statement_line l,makemyday_order o,makemyday_order_line i where  i.order_id=o.id and o.state='paid' and l.statement_id=s.id and l.makemyday_statement_id=o.id and s.id=%d"""%(o.id)
...
class report_makemyday_user_product(osv.AbstractModel):
    _name = 'report.point_of_sale.report_usersproduct'
    _inherit = 'report.abstract_report'
    _template = 'point_of_sale.report_usersproduct'
    _wrapped_report_class = makemyday_user_product
